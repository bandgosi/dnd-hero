...
"D&D Герой" — a Russian-language D&D 5e character sheet Progressive Web App. The entire application is a single self-contained `index.html` (~6,000 lines: CSS, HTML, and JavaScript) with no framework, no dependencies, no build step. Standalone Playwright regression checks live in `tests/responsive.cjs` (layout) and `tests/redesign.cjs` (v40 master/detail behaviour). All D&D content (spells, items, rules) and even the QR encoder are embedded locally, so the app works fully offline.

## Development

There is no build or lint step. For the optional responsive regression check, install Playwright in your development environment (`npm install --no-save playwright`, `npx playwright install chromium`) and run `node tests/responsive.cjs` and `node tests/redesign.cjs`. Each starts a temporary Python HTTP server (ports 8000 / 8001); the first checks 11 viewport sizes and all tabs, modal bounds, resizing, persistence and dice, the second covers spell/inventory search, filters, detail navigation, slots, preparation, concentration and the one-time save snapshot. `CHROMIUM_EXECUTABLE` may point to an existing Chromium executable. No test dependency is loaded by the application. To run locally, serve the directory over HTTP (the service worker does not work from `file://`):
...
- `backup/` — snapshot of the last version before the current major change (`index.html`, `sw.js`, `manifest.json`), kept for rollback; currently v39 (pre-v40 "readable grimoire" redesign — the v33 pre-★Final snapshot remains in git history). Commit `caedf29` on `main` is the same v39 state. To roll back an already-published version, restore these files and give `CACHE_NAME` a new unique value so installed PWAs pick up the old UI as an update.
...
- Optional `REVIEW_SCREENSHOTS=/absolute/path` makes `tests/redesign.cjs` write phone and unfolded review images from the real app.
