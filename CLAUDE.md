...
- `backup/` — snapshot of the last version before the current major change (`index.html`, `sw.js`, `manifest.json`), kept for rollback; currently v39 (pre-v40 "readable grimoire" redesign — the v33 pre-★Final snapshot remains in git history). Git tag `v39` marks the same commit. To roll back an already-published version, restore these files and give `CACHE_NAME` a new unique value so installed PWAs pick up the old UI as an update.
