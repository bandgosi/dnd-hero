...
**When changing `index.html`, bump `CACHE_NAME` in `sw.js`** (e.g. `dnd-hero-v38` → `dnd-hero-v39`) and the "версия N" label on the welcome screen in `index.html` (kept in sync manually). Currently v39. The document itself is network-first; every other asset is cache-first, so installed PWA clients keep the old assets until the cache version changes.

The update handshake is deliberate: `sw.js` does **not** call `skipWaiting()` on install. A new worker installs and waits; the page shows the "Доступно обновление" toast, and only when the player presses ОБНОВИТЬ does `applyUpdate()` post `'skipWaiting'` to the waiting worker, whose activation fires `controllerchange` and reloads once. The `controllerchange` handler ignores the very first fire (no previous controller) — otherwise a fresh install reloads mid-startup and throws away a pending `#i=` import.
...
- App icons are quantised to a 256-colour palette (v39) — regenerating one from vector means re-quantising, or `icons/icon-512.png` grows back from 52 KB to ~107 KB for no visible gain.
...
- `CLASS_GUIDES` — per-class guide tab content. **All 12 classes must have an entry**: `renderClassGuide()` falls back to `CLASS_GUIDES['Колдун']`, so a missing class silently shows the warlock page as if it were correct (this is what happened to Плут/Варвар/Монах until v39).
...
- Sharing: an inline dependency-free QR encoder (`QR.generate`, byte mode / EC-L / versions 1–25, ~1273 bytes max) renders a `#i=`-URL QR when the pruned payload fits; otherwise a copyable base64 code. `compactCharPayload()` strips photo/history/computed slots/empty values **and reduces inventory items and spells to bare names** (`{n,q,a}` / `{n,p}`) — everything else is rehydrated from the local database by `expandCompactLists()` inside `parseCharPayload()`. Payloads that predate v39 store full objects and pass through untouched (detected by the absence of `n`). Keep both sides in sync: a new per-item field that the player can edit must be added to the compact form, or it is lost on share. `handleShareHash()` auto-imports from `#i=` on load. `printCharacter()` fills `#print-sheet` and calls `window.print()` (print-only `@media print` layout).

## Conventions

- All UI text, data, and most code comments are in Russian; keep new UI strings in Russian.
- The theme is dark, D&D Beyond-inspired (graphite surfaces, red accents, muted gold): palette lives in `:root`, including the `--magic*` purple ramp used by everything spell-related. Note that CSS custom properties do **not** work inside `url("data:image/svg+xml…")` backgrounds or SVG presentation attributes (`stroke="#C9A03A"`), so those hexes stay literal. **Variable names are historical** — `--parchment` and `--ink` are now LIGHT text colors on dark surfaces; backgrounds use `--bg`/`--surface`/`--card`; thin borders use `--line`. Do not reintroduce light backgrounds under `var(--parchment)`.
