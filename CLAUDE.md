...
- `images/` — AI-generated art, compressed (keep total small; precache in `sw.js`): `welcome-art.jpg` (full-screen welcome background: d20 on a grimoire, dark top for the title, button bottom), `bg-texture.jpg` (subtle hex/sigil tile under tab content, applied in `.content` beneath a translucent surface-color layer so tile seams stay invisible), `frame.png` (ornate gold frame overlaid on the portrait via `.char-portrait::after`; keyed out from a checkerboard source), `crests/*.png` (12 engraved class emblems, filenames in `CLASS_CREST_IMG`; used via `crestImgTag()` in the class banner and guide card — the stroke-SVG `CLASS_CRESTS` remain only as fallback), `empty/*.png` (empty-state art: spells/inventory/features/rolls), `items/*.png` (30 engraved gold-line inventory item pictograms on transparent background; keyword→filename map in `ITEM_ICONS`, rendered via `itemIconImg()` — old saves store emoji in `item.icon`, the key is recomputed from the name), `ui/*.png` (30 engraved gold-line interface glyphs replacing the old colour emoji in buttons, section titles, subtabs and knowledge-base cards; inserted inline via `uiIc(name)` in JS templates or a literal `<img class="ui-ic">` in static HTML — `.ui-ic` is sized in `em`, so it scales with the parent's `font-size`).
